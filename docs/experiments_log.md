...
---
---
## 2026-01-10 - 물리적 의미 유지 정규화 구현 [claude-session002]

### 작업 내용
- Agent: claude
- Session: 002
- 물리량별 통일 정규화 전략 수립
- 실제 데이터 기반 min/max 값 산출
- 정규화/역정규화 함수 구현

### 정규화 스크립트
- `scripts/normalize.py` - 정규화/역정규화 함수

### 기대 효과
- 확인사항: 같은 물리량 변수 간 직접 비교 가능 여부
- 목표: Diffusion 모델 학습에 적합한 [0, 1] 범위 정규화
- 배경: 임의 기준이 아닌 실제 데이터 기반 정규화 필요
...
| 물리량 | 변수 | 통일 범위 | 정규화 |
|--------|------|-----------|--------|
| 온도 | TCLAD_HOT, TWCR, TWSG | [235, 3755]°C | (x-235)/3520 |
| 수위 | zw_vessel, RWST | [0, 13.55]m | x/13.55 |
| 유량 | HPSI, WAFWSG | [0, 100.3]kg/s | x/100.3 |
| 압력 | PPS | [30.9K, 19.3M]Pa | log10 스케일 |
| 기타 | ZWDC2SG | [0, 15.02] | x/15.02 |
...
- 플랜 문서: `C:\Users\ADMIN\.claude\plans\synchronous-petting-coral.md`

### 다음 단계
- DataLoader 구현 (정규화 적용)
- Diffusion forward process 구현
---
---
## 2026-01-11 - 정규화된 H5 파일 생성 [claude-session003]

### 작업 내용
- Agent: claude
- Session: 003
- 학습 속도 향상을 위해 사전 정규화된 H5 파일 생성
- CLAUDE.md 파일 설명 업데이트

### 생성 스크립트
- `scripts/create_normalized_h5.py`

### 기대 효과
- 확인사항: 학습 시 on-the-fly 정규화 오버헤드 제거
- 목표: 배치 로딩 속도 향상
- 배경: 정규화 연산이 매 배치마다 반복되는 비효율 해소

### 결과
| 파일 | 용도 | 특징 |
|------|------|------|
| `tspred_v2_cleaned.h5` | 분석용 | 원본 물리값 유지 |
| `tspred_v2_normalized.h5` | 학습용 | [0,1] 범위, float32 |

- 입력: 24D → 47D (23값 + 24마스크)
- 출력: (T, 9) → [0, 1] 범위
- 메타데이터: 정규화 상수 포함
...
## 2026-01-10~11 데이터 정규화 작업 히스토리

### 전체 흐름
```
원본 데이터 (tspred_v2_new.h5)
    ↓ [1] 전처리 (preprocess_h5.py)
    ↓ - 중복 188건 제거 (FirstData+SBO/PSV)
    ↓ - 입력값 1000 → 72 변환
전처리 데이터 (tspred_v2_cleaned.h5)
    ↓ [2] 정규화 (normalize.py)
    ↓ - 물리량별 통일 min/max
    ↓ - 마스크 채널 추가
정규화 데이터 (tspred_v2_normalized.h5)
```

### [1] 전처리 단계 (2026-01-10)
| 항목 | 내용 |
|------|------|
| 중복 제거 | FirstData+SBO 187건, FirstData+PSV 1건 |
| 특수값 변환 | 1000 → 72 (226,672회) |
| 결과 | 34,774 → 34,586건 |

### [2] 정규화 단계 (2026-01-10~11)

#### 입력 정규화 (24D → 47D)
| 변수 | 인덱스 | 처리 |
|------|--------|------|
| 시간 | 0-15, 19-21 | ÷259,200 (72h) |
| 플래그 | 16-18 | 그대로 |
| SEAL_AREA | 22 | ÷5 |
| 케이스번호 | 23 | 제외 |
| 특수값 -1 | - | 마스크 채널 |
...
### 핵심 설계 결정
1. **물리량 통일**: 같은 물리량(온도, 수위, 유량)은 동일 범위로 정규화 → 변수 간 비교 가능
2. **실제 데이터 기반**: 임의 기준이 아닌 실제 min/max 사용
3. **마스크 채널**: 특수값 -1을 별도 채널로 분리 → 정보 손실 방지
4. **Log 스케일 (압력)**: 큰 동적 범위 (30K~19M Pa) 처리
5. **파일 분리**: 분석용(원본) / 학습용(정규화) 분리
